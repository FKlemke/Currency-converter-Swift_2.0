...
class ViewController: UIViewController {
  ...
  @IBAction func updateRates(sender: AnyObject) {
    ...
    //save default values NSUserDefault
    saveUserDefaults()
  ...
  func readDefaultData(){
    if let defaultData = defaults.objectForKey("conversionRates") {
      conversionRates = defaultData as! [String : Double]
      print("success maybe")
      
      //update displays //boilerplate
      updateDisplayEURUSD()
      updateDisplayEURGBP()
    }
  }
  
  func updateDisplayEURUSD(){
    if let eurUsdRate = conversionRates["EURUSD"]{
      eurUsdConversionRate.text = String(eurUsdRate)
    }
  }
  
  func updateDisplayEURGBP(){
  ...
  func saveUserDefaults(){
    defaults.setObject(conversionRates, forKey: "conversionRates")
  }
